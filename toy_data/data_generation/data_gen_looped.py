...
observable_data_path = '/Users/jakubpazio/Imperial/Master Project/new-physics/new-physics/toy_data/data_generation/interpolation/interp_2022_2_23_15_01_8'
...
for c9_t in np.linspace(c9_min, c9_max, c_samples):
    temp = pd.DataFrame({
    ...
    random_data = pd.concat([random_data, temp])



#%%
df_len = random_data.shape[0]
random_data['BR_rnd'] = np.random.uniform(min_br, max_br, df_len)
random_data['dBR_rnd'] = np.random.uniform(min_dbr, max_dbr, df_len)
...
completely_filtered_data.to_csv(f'/Users/jakubpazio/Imperial/Master Project/new-physics/new-physics/toy_data/data_generation/datasets/mixed_data_{date.year}_{date.month}_{date.day}_{date.hour}.csv', index=False)
...
# while len(completely_filtered_data[1]) < number_data_points:
